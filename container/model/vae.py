...
class VAE_UNET(nn.Module):
    ...
    def __init__(self, layers_down, layers_up, layers_add, layers_vae, layers_add_vae,
                 ...
                 vae_dimensionality=256, vae_channels_out=4, group_norm_channels=8 ):
        ...
        for j,i in enumerate(layers_vae) :
            (inception_layers, out_layer_chan) = i
            self.mods_vae.append(InceptionModule(in_layer_chan_vae, inception_layers, out_layer_chan,
        ...
        for i in layers_add_vae:
            ...
            (inception_layers, out_layer_chan) = i
            _add_vae.append( InceptionModule(in_layer_chan_vae, inception_layers, out_layer_chan,
                               leaky=leaky, norm_fn='group', 
                               groups=group_norm_channels, rectify=False ))
            in_layer_chan_vae = out_layer_chan
        ...
        if layers_add_group is not None:
            ...
            for i in layers_add_group:
                (inception_layers, out_layer_chan) = i
                _add_group.append( InceptionModule(in_layer_chan_group,
                                    inception_layers, out_layer_chan,
                                    leaky=leaky, norm_fn='group', 
                                    groups=group_norm_channels, rectify=False ))
            ...
            print(f"add_group: {in_layer_chan_group=},{out_layer_chan=}")
        else:
...
def init_model(desc, parallel=False , patch_sz=32, cpu=False):
    """
    ...
    model = VAE_UNET(desc.get('downstream',
                      [
    ...
    if not cpu:
        model.cuda()
    if 'load' in desc:
        load_model(model, desc['load'], cpu=cpu)
    if 'import' in desc:
        import_parameters(model, desc['import'], cpu=cpu)
    if parallel:
      #model = nn.parallel.DistributedDataParallel(model)
      pass # HACK - move parallel out of model initialization
    # TODO: fake parallel when it is not present?
    return model
