...
## Effect sizes
fpath       <- here("data/rds/adni-bl_effect-sizes_hc-msrs_dx_labs.rds")
if (file.exists(fpath)) {
  efflabs   <- read_rds(fpath)
} else {
  here('code/analysis/effect-sizes_hc-hvr-havr-havas_adni-bl.R') |> source()
}
...
for (measure in DT[, levels(MEASURE)]) {
  fnames      <- sprintf("plots/adni-bl_similarity_%s-adj_dx.%s",
  ...
  if (!file.exists(fnames[1]) || !file.exists(fnames[2]) || ReDoPlots) {
    ...
    g <- ggpairs(dt, columns = 3:length(dt),
      ...
      labs(title = sprintf("Head-size adjustment: %s", measure),
