...
## CH vs AD
fnames      <- sprintf("data/rds/adni-bl_effect-sizes_fs-hc-msrs_dx%s.rds",
                       c("", "_labs", "_sims")) |> here()
...
p1 <- ggplot(aes(x = AVG, colour = DX), data = DT[MEASURE == "HC"]) +
  ...
  labs(x = NULL, y = NULL, colour = NULL, subtitle = "Hippocampal volume (FS)")
...
p2 <- ggplot(aes(x = AVG, colour = DX), data = DT[MEASURE == "HVR"]) +
  ...
  labs(x = NULL, y = NULL, colour = NULL,
       subtitle = "Hippocampal-to-Ventricle ratio (FS)")

p <- grid.arrange(p1, p2, nrow = 2)
fnames <- sprintf("plots/adni-bl_fs-hc-msrs_effsizes-dx.%s",
                  c("png", "tiff")) |> here()

if (!file.exists(fnames[1]) || ReDoPlots) {
  fnames[1] |>
  ggsave(p, width = 12, height = 5, units = "in", dpi = 600)
}

#if (!file.exists(fnames[2]) || ReDoPlots) {
  #fnames[2] |>
  #ggsave(p, width = 12, height = 5, units = "in", device = "tiff", dpi = 600)
#}
