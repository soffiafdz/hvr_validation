...
volumes     <- fs_vols[, .(PTID, SCANDATE, LHC, RHC, LCSF, RCSF)]

rm(fs_vols)

# Merge ICC and volumes
volumes     <- icc_scales[volumes, on = .(PTID, SCANDATE)
                          ][PTID != "<ptid>...."] # Outlier with HUGE head (?)
rm(icc_scales)
...
volumes[, c("LHC", "RHC", "LCSF", "RCSF") := NULL]
...
## HVR
volumes[!ADJ %in% c("stx", "prop"), HVR := HC / (HC + CSF)]
...
## Export RDS
write_rds(volumes, here("data/rds/adni-bl_fs-hc-msrs_icv-adjusted.rds"))
