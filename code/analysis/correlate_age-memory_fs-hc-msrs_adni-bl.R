...
fpaths <- sprintf("data/rds/adni-bl_fs-hc-msrs_corrs_%snon-parametric.rds",
...
## HC
corr.dt[, ADJ := factor(ADJ, labels = c("Unadj.", "STX", "Proport.",
...
for (i in seq_along(msrs)) {
  p  <- ggplot(corr.dt[MEASURE == msrs[i]], aes(DX, R, colour = ADJ)) +
    ...
    labs(title = sprintf("Correlations: %s (FS)", msrs[i]),
         x = "TIV adjustment method",
         y = "Spearman's rho",
         caption = "* p < 0.05; ** p < 0.01; *** p < 0.001")

  sprintf("plots/adni-bl_fs-%s-adj_corrs.png",
          stringr::str_to_lower(msrs[i])) |>
    here() |>
