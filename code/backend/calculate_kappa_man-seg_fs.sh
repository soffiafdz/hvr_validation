...
recode() {
	itk_resample \
		--labels \
		--clobber \
		--lut-string "11 1; 13 2; 21 3; 23 4" \
		--like $3 \
		$1 $2
}
...
for segmented in $FS_LABS/*hcilvc.mnc
do
	bname=$(basename $segmented)
	id=${bname#tal_as_}
	id=${id%%_*}

	segmented_recoded=$TMPDIR/${id}_fs.mnc
	manual=$MANUAL_LABS/tal_${id}_hp_csf_ld.mnc
	manual_recoded=$TMPDIR/${id}_manual.mnc

	recode $segmented $segmented_recoded $manual
	recode $manual $manual_recoded $manual
