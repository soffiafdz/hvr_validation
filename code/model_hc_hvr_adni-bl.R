...
# Train/test split
set.seed(1618)
train_idx     <- createDataPartition(amy_positive[, HC], p = .8, list = FALSE)
train_dt      <- amy_positive[ train_idx]
test_dt       <- amy_positive[-train_idx]
rm(train_idx)
...
train_dt[, `:=`(HC_z      = (HC - hc_mean) / hc_std,
...
test_dt[, `:=`(HC_z      = (HC - hc_mean) / hc_std,
               ...
               HC_norm_z = (HC_norm - hc_norm_mean) / hc_norm_std,
               HVR_z     = (HVR - hvr_mean) / hvr_std)]
rm(hc_mean, hc_std, hc_stx_mean, hc_stx_std,
   hc_norm_mean, hc_norm_std, hvr_mean, hvr_std)
...
covars <- c("DX", "AGE", "PTGENDER", "PTEDUCAT",
...
res_tst[[1]]  <- postResample(predict(lm_hc, test_dt),
                              test_dt[, HC_z])
...
res_trn[[5]]  <- data.table(glm_hc$results)[select == glm_hc$bestTune[[1]],
                                            .(RMSE, Rsquared, MAE)]
res_tst[[5]]  <- postResample(predict(glm_hc, test_dt),
                              test_dt[, HC_z])
