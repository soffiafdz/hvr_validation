...
QC_DIR=${BASE_DIR}/plots/qc_adni-s2
...
mapfile -t IDS < ${BASE_DIR}/lists/adni_hcvc-segm.lst
IDS=( $(shuf -e "${IDS[@]}") )
...
do
	...
	sess=$(printf $id | cut -d, -f2)
	segmentation=$(printf $id | cut -d, -f4)
	in_mri=$(printf $id | cut -d, -f5)

	# Skip 1st segmentation since it is already done
	[[ $segmentation == 1 ]] && continue

	# out_bname
	sess=${sess//-/}
	bname=adni_${sub}_${sess}_hcvc-cnn

	# HCVC CNN
	out_simple=${OUT_DIR}/adni-s2_cnn_hcvc/${bname}.mnc
